...
namespace FraudMonitoringSystem.Repositories.Customer.Implementations.Admin
{
    public class SystemUserRepository : ISystemUserRepository
    {
        ...
        public async Task<int> CountActiveByRoleIdAsync(string roleId)
        {
            return await _context.SystemUsers
                .CountAsync(u =>
                    u.RoleId == roleId &&
        ...
        {
            var max = await _context.SystemUsers
                .Select(x => x.SystemUserCode)
                .Where(c => c.StartsWith("B"))
                .Select(c => int.Parse(c.Substring(1)))
                .DefaultIfEmpty(299)
                .MaxAsync();
        ...
        {
            Task<int> CountByRoleIdAsync(string roleId); // ✅ reuse existing method
        }
    }
}